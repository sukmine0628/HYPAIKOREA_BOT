...
// F열에 '관리자'면 승인 담당자 (0-based index: A=0, ..., F=5)
...
/** 승인 메뉴(안내) — 실제 처리는 DM 버튼 or 승인 리스트 구현 시 확장 */
bot.action('purchase_approve', async ctx => {
  ...
  if (!ok) {
    return ctx.reply('접근 권한이 없습니다. 관리자에게 문의하세요.');
  }
  await ctx.reply('승인/반려는 DM 알림의 버튼으로 처리하거나, 곧 제공될 승인 목록에서 선택해 주세요.');
});

/** 내 요청 보기(대기중만 간단히 보여줌) */
bot.action('my_requests', async ctx => {
  ...
  try {
    ...
    const rows = res.data.values || [];
    const mine = rows.filter(r => String(r[2]) === String(ctx.chat!.id));
    if (!mine.length) return ctx.reply('등록된 구매 요청이 없습니다.');

    const waiting = mine.filter(r => (r[8] || '') === '대기중');
    if (!waiting.length) return ctx.reply('대기 중인 내 요청이 없습니다.');

    const lines = waiting.map(r => `• ${r[0]} / ${r[3]} x ${r[4]} / ${Number(r[5]||'0').toLocaleString()}원`);
    await ctx.reply(`대기 중인 내 요청\n${lines.join('\n')}`);
  } catch (e: any) {
    console.error('MY_REQUESTS_ERROR', e?.response?.data || e);
    await ctx.reply('조회 중 오류가 발생했습니다.');
  }
});
  ...
  try {
    ...
    if (state) {
      ...
      if (state.stage === 'note') {
        ...
        for (const m of managers) {
          try {
            await bot.telegram.sendMessage(m.chatId, msg, kb);
          } catch {}
        }
